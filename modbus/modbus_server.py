...
def build_args():
    p = argparse.ArgumentParser(description="Modbus multi-slave simulator.")
    p.add_argument("--update-interval", type=float, default=0.005)
    p.add_argument("--print-interval", type=float, default=0.25)
    p.add_argument("--host", type=str, default="localhost")
    p.add_argument("--port", type=int, default=5020)
    return p.parse_args()

# initial registers for two slaves (16 registers each i.e. 8 signals)
...
# here if value is 22.34, then high=2234, low=0, because we scale by 100
# but if we use float binary representation, we would use two registers per signal
# e.g. 22.34 -> high=16921, low=17472 (because 22.34 as float32 is 0x41B26666)

# Create separate slave contexts
...
# modbusSlaveContext -> define the memory map for a single slave
# hr -> holding registers

# Map unit id -> slave context; IMPORTANT: use single=False
stores = {
    1: store1,
    2: store2
}
...
def simulate_slave(unit_id, base_highs, update_interval, jitter_scale=0.02):
    amplitudes = [50, 30, 100, 10, 5, 80, 120, 20]
    periods = [8, 6, 12, 10, 3, 9, 7, 11]
    t0 = time.time()
    i = 0
    while True:
        ...
        for idx, base in enumerate(base_highs):
            ...
            new_regs.append(new_high)
            new_regs.append(0)
        # write to holding registers of given unit_id
...
def pretty_monitor(print_interval):
    ...
    while True:
        ...
        for unit_id in stores.keys():
            ...
            print("="*60)
            print(f"Slave {unit_id} @ {time.strftime('%H:%M:%S', time.localtime(now))}")
            for i in range(0, 16, 2):
                idx = i // 2
                raw_high = regs[i]
                scaled = raw_high / 100.0
                print(f"{signal_names[idx]:<18} | {scaled:8.3f}  (regs {i},{i+1} => {raw_high})")
            print("="*60)
        time.sleep(print_interval)
...
if __name__ == "__main__":
    ...
    # start simulators per slave
    base_highs_1 = [initial_regs_slave1[i] for i in range(0, len(initial_regs_slave1), 2)]
    base_highs_2 = [initial_regs_slave2[i] for i in range(0, len(initial_regs_slave2), 2)]
    Thread(target=simulate_slave, args=(1, base_highs_1, args.update_interval), daemon=True).start()
    Thread(target=simulate_slave, args=(2, base_highs_2, args.update_interval), daemon=True).start()
